...
3. In ```- (BOOL) update:(Words *)word changeEditTime:(BOOL)changeEditTime; and delete SAFE_STR(word.word)``` of DatabaseService.m
Change ```@"UPDATE SET word='%@', result='%@', description='%@', favorites='%@', edited='%@' WHERE _id=%ld"```
